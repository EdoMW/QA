...
In the time periods between days 51-100 there were no alarms. I've tried simulating for more days and the first alarm came on the 131 day (181 from day 1). If had an alarm, it would have been a false alarm. the reason for that is that I know the real distribution of the data because I have generated it and I know there the mean and the standard deviation hasn't changed.
...
```
5.2. The name of the model which we generated the data from is: Accelerated Life model
```{r, echo=TRUE}
paste("5.3")
paste(beta)
paste("The real effect size of each attribute is:")
for (i in 1:10)
{
...
to_print <- paste("The expected life predicted is:", excpted_life)
print(to_print )
```
